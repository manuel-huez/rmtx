...
func EnsureHostPKI(stateDir, serverName string) (HostPKI, error) {
	...
	if rotateServer {
		serverCertPEM, serverKeyPEM, err := generateServerKeyPair(current.CACertPEM, current.CAKeyPEM, serverName)
		if err != nil {
			return HostPKI{}, err
		}
...
func hostPKIRotation(pki HostPKI) (bool, bool, error) {
	...
	serverCert, err := parseCertPEM(pki.ServerCertPEM)
	if err != nil {
	...
	if _, err := tls.X509KeyPair(pki.ServerCertPEM, pki.ServerKeyPEM); err != nil {
		return false, true, nil
	}
...
func ClientTLSConfig(certPEM, keyPEM []byte, expectedFingerprint string) (*tls.Config, error) {
	...
	cfg := &tls.Config{
		...
		VerifyPeerCertificate: func(rawCerts [][]byte, _ [][]*x509.Certificate) error {
			if len(rawCerts) == 0 {
				return errors.New("host certificate missing")
			}
			...
			for i, rawCert := range rawCerts {
				...
				if err != nil {
					return fmt.Errorf("parse host certificate %d: %w", i, err)
				}
