...
func runContextList(ctx context.Context, args []string) int {
	...
	contexts, err := app.RunListContexts(
		ctx,
		cwd,
		app.RemoteParams{
			AddressOverride:  *hostAddr,
...
func runContextDelete(ctx context.Context, args []string) int {
	...
	result, err := app.RunDeleteContexts(
		ctx,
		cwd,
		app.ContextDeleteParams{
			AddressOverride:  *hostAddr,
			ConfigPath:       *cfgPath,
			TokenOverride:    *token,
			DiscoveryTimeout: *discoveryTimeout,
	...
	for _, context := range result.Deleted {
...
func runContextPrune(ctx context.Context, args []string) int {
	...
	if len(result.Deleted) == 0 {
		fmt.Fprintln(os.Stdout, "deleted\t0")
	}
...
func resolveTTYMode(force, disable bool) (app.TTYMode, error) {
	if force && disable {
		return app.TTYAuto, fmt.Errorf("--tty and --no-tty cannot be used together")
	}
